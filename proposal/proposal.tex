...
\title{Transfer Learning Project Proposal}
\author{%
  Chenyu Tian \\
  Columbia University\\
...
\section{Method / Algorithm to be Investigated}
...
\begin{itemize}
    \item \textbf{Model Structure:} Use a pre-trained feature extractor followed by a superclass classification head and a subclass classification head. Train the network jointly using a combined cross-entropy loss from both heads.

    \item \textbf{Training and Inference:} Select a \textbf{probability-based threshold method} (e.g. Maximum Softmax Probability(MSP)~\cite{hendrycks2018baselinedetectingmisclassifiedoutofdistribution}) for novel superclass and subclass classification, and select a \textbf{conditional constraint method} (e.g., attention mechanism for soft constraint during training + masking strategy for hard constraint during inference). Predict the superclass then the subclass based on the probability-based threshold method. Apply the constraint method to the subclass classifier to avoid inconsistent labeling.
\end{itemize}
