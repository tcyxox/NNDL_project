...
def save_split(name, features, text_features, super_labels, sub_labels):
    """辅助函数：保存切分好的数据集"""
    torch.save(features, os.path.join(CONFIG["output_dir"], f"{name}_features.pt"))
    if text_features is not None:
        torch.save(text_features, os.path.join(CONFIG["output_dir"], f"{name}_text_features.pt"))
...
if __name__ == "__main__":
    ...
    text_features = torch.load(os.path.join(CONFIG["feature_dir"], "train_text_features.pt"))
    ...
    # 校验图像和文本特征数量对齐
    assert features.shape[0] == text_features.shape[0], "图像和文本特征数量不一致！"
    ...
    save_split("val", val_feat[perm_val], val_text[perm_val], val_super[perm_val], val_sub[perm_val])

    # --- 构建 Test Set (混合，已知+未知) ---
    test_feat = torch.cat([features[idx_test_known], features[idx_test_novel]])
    test_text = torch.cat([text_features[idx_test_known], text_features[idx_test_novel]])
    ...
    save_split("test", test_feat[perm_test], test_text[perm_test], test_super[perm_test], test_sub[perm_test])
