...
We first establish a baseline using a Linear Dual Head model trained with Cross-Entropy (CE) loss. We investigate the impact of threshold determination methods (Quantile vs. Z-Score), Temperature Scaling, and the proposed SE-style Feature Gating.

As shown in Table \ref{tab:ablation}, the \textbf{Z-Score} method significantly outperforms the non-parametric Quantile method, suggesting that the OOD scores follow a Gaussian-like distribution. Applying \textbf{Temperature Scaling} further improves the distinction between known and unknown samples. Finally, the \textbf{Gated Dual Head} architecture, which utilizes SE-Attention to recalibrate features, achieves the best performance, confirming that hierarchical constraints help shape the feature space.
...
Table \ref{tab:scoring_comparison} reveals a trade-off between metrics. \textbf{CE + MSP} yields the highest AUROC, indicating the best ranking capability for OOD detection. While \textbf{BCE + MaxSigmoid} achieves the highest Overall and Unseen accuracy, additional experiments highlight a critical limitation: it exhibits excessive confidence on known superclasses. Although introducing novel superclasses into the validation set partially mitigates this bias, the overconfidence persists. In contrast, MSP demonstrates superior robustness without such calibration issues. \textbf{Energy} scores require significantly lower temperatures ($T=0.02$) to be effective but generally underperform in this hierarchical setting compared to probability-based methods.
...
\subsection{Impact of Validation Strategy}

We analyze the impact of the validation set composition on threshold calibration. In the \textit{Strict} setting, thresholds are derived solely from known data (Z-Score). In the \textit{Guided} setting, the validation set includes unknown samples, allowing for the \textit{Intersection} method.

As expected, access to unknown samples during validation significantly boosts performance. For the CE+MSP model, switching from Strict to Guided validation significantly improve the performance, demonstrating that the distribution of validation outliers generalizes well to the test set.

\begin{table}[h]
  \caption{Impact of Validation Strategy on Subclass Performance (Model: CE + MSP).}
  \label{tab:val_strategy}
  \centering
  \resizebox{\columnwidth}{!}{
  \begin{tabular}{lccccc}
    \toprule
    \bf Validation Strategy & \bf Threshold Method & \bf Overall Acc. & \bf Seen Acc. & \bf Unseen Acc. & \bf AUROC \\
    \midrule
    Strict (Known Only) & Z-Score & 73.83\% ± 3.88\% & \textbf{88.03\% ± 1.30\%} & 59.79\% ± 8.48\% & \textbf{0.8821 ± 0.0234} \\
    Guided (Known + Novel) & Intersection & \textbf{79.57\% ± 1.55\%} & 72.68\% ± 2.61\% & \textbf{84.90\% ± 4.12\%} & 0.8663 ± 0.0119 \\
    \bottomrule
  \end{tabular}
  }
\end{table}
