...
Used in \textit{Strict Open-Set Validation}. This method assumes no prior knowledge of the unknown distribution. Given a target recall rate $r$ (e.g., $r = 0.95$) for known classes, we set $\tau$ as the $(1-r)$-th quantile of the scores from the known validation set $\mathcal{D}_{\text{val}}^{\text{known}}$:
\begin{equation}
...
\paragraph{Distribution Intersection (Full Validation).}
Used in \textit{Guided Open-Set Validation}. When the validation set contains both known and novel samples ($\mathcal{D}_{\text{val}}^{\text{known}} \cup \mathcal{D}_{\text{val}}^{\text{novel}}$), we can explicitly model the boundary between the two distributions. We calculate $\tau$ as the intersection point of the empirical distributions, effectively balancing the FPR and FNR:
\begin{equation}
...
\paragraph{Strict Open-Set Validation:} \textit{Novel} samples appear only in the Test set.
...
\paragraph{Guided Open-Set Validation:} \textit{Novel} samples appear in the Test and Validation set.
