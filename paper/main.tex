...
We analyze the impact of the validation set composition on threshold calibration. In the \textit{Strict} setting, thresholds are derived solely from known data (Z-Score). In the \textit{Guided} setting, the validation set includes unknown samples, allowing for the \textit{EER} method.
...
\begin{table}[h]
  ...
  \begin{tabular}{lccccc}
    ...
    Guided (Known + Novel) & EER & \textbf{79.57\% ± 1.55\%} & 72.68\% ± 2.61\% & \textbf{84.90\% ± 4.12\%} & 0.8663 ± 0.0119 \\
    \bottomrule
  \end{tabular}
  }
\end{table}
