...
Problems and proposed solutions addressed in this method include:
\begin{enumerate}
    ...
    \item \textit{How do we determine whether the label is novel or known?}
    \\
    \textbf{Answer:} We apply a simple probability-based threshold block with the softmax.

    \item \textit{How do we let the superclass classification impact the subclass classification?}
    \\
    \textbf{Answer:} We apply a soft constraint during training and a hard constraint during inference.
\end{enumerate}
...
\begin{itemize}
    \item Select a \textbf{probability-based threshold method} (e.g., Maximum Softmax Probability (MSP)~\cite{hendrycks2018baselinedetectingmisclassifiedoutofdistribution}) for novel superclass and subclass classification.
    \item Select a \textbf{soft constraint method} used in training (e.g., Channel-Wise Attention ~\cite{hu2019squeezeandexcitationnetworks}), select a \textbf{hard constraint method} used in inference (Masking strategy).
