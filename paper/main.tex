...
A known issue with MSP and ODIN is their reliance on the Softmax function, which loses the magnitude information of the logits. Logit-based scoring methods resolve this problem.
...
While the MSP framework maintains consistency between its scoring method and the Cross-Entropy training objective (as both rely on softmax), Logit-based OOD detection methods introduce a \textbf{theoretical inconsistency} with Cross-Entropy. To resolve this, we replace the Softmax normalization with \textbf{class-wise Sigmoid activations} and adopt \textbf{Binary Cross-Entropy (BCE)} loss. This treats the multi-class classification problem as a collection of independent \textbf{one-vs-rest (OvR)} binary tasks~\cite{shu2017docdeepopenclassification}.
...
This strategy simulates a strict scenario where no outliers are available during the development phase and is used in Post-hoc Method. Thresholds must be determined using statistical methods based solely on the distribution of known data.
...
\begin{table}[h]
  \caption{Ablation study on model architecture and threshold strategies.}
  \label{tab:ablation}
  \centering
...
\begin{table}[h]
  ...
  \begin{tabular}{lccccc}
    \toprule
    \bf Method (Loss + Score) & \bf Temperature & \bf Overall Acc. & \bf Seen Acc. & \bf Unseen Acc. & \bf AUROC \\
    \midrule
...
In this section, we focus exclusively on the CE + MSP method to investigate the impact of different calibration strategies. We also apply fine-tuned hyper-parameters and train based on a larger base model (ViT-L/14) to achieve optimal result among post-hoc score-based methods for OSR.

We employ a nested calibration protocol for rigorous evaluation. The train set is further partitioned into sub-train and validation sets across multiple random seeds, and the calibrated thresholds are averaged to reduce variance. The final model is then retrained on the full train set using the averaged thresholds before testing.
...
\begin{table}[h]
  ...
  \begin{tabular}{lccccc}
    \toprule
    \bf Calibration & \bf Threshold & \bf Overall Acc. & \bf Seen Acc. & \bf Unseen Acc. & \bf AUROC \\
    \midrule
    Strict & Z-Score & 71.98\% ± 3.23\% & 90.21\% ± 0.66\% & 50.34\% ± 6.97\% & 0.8799 ± 0.0276 \\
    Guided & EER & 79.05\% ± 3.59\% & 81.52\% ± 2.53\% & 76.00\% ± 7.42\% & 0.8799 ± 0.0276 \\
    Guided (L/14) & EER & \textbf{83.02\% ± 3.69\%} & \textbf{84.84\% ± 2.84\%} & \textbf{80.74\% ± 8.65\%} & \textbf{0.9117 ± 0.0318} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
\subsection{Final Evaluation on Leaderboard}

\paragraph{Configuration:}
\begin{itemize}
    \item \textbf{Feature Extractor:} CLIP ViT-H/14 (1024-dim features)
    \item \textbf{Architecture:} SE Feature Gating, Hierarchical Masking
    \item \textbf{Training:} Cross-Entropy loss, 75 epochs, 1e-3 learning rate
    \item \textbf{Inference:} OpenMax (Weibull tail=5, $\alpha$=3)
    \item \textbf{Validation Set:} 10\% unknown subclass
    \item \textbf{Calibration: } Z-Score (k=1.645) for super class, EER for subclass
\end{itemize}

\begin{table}[h]
  \caption{Leaderboard Results Comparison}
  \label{tab:leaderboard}
  \centering
  \resizebox{\columnwidth}{!}{
  \begin{tabular}{lcccccc}
    \toprule
    \multirow{2}{*}{\bf Model} & \multicolumn{3}{c}{\bf Superclass Accuracy (\%)} & \multicolumn{3}{c}{\bf Subclass Accuracy (\%)} \\
    \cmidrule(lr){2-4} \cmidrule(lr){5-7}
    & Overall & Seen & Unseen & Overall & Seen & Unseen \\
    \midrule
    Baseline (CLIP/B-32) & 80.42 & 99.14 & 32.91 & 13.10 & 60.75 & 0.24 \\
    \midrule
    Ours (Internal Val) & 99.36 ± 0.34 & 99.36 ± 0.34 & N/A$^\dagger$ & 89.23 ± 2.74 & 89.18 ± 1.41 & 89.36 ± 4.62 \\
    \textbf{Ours (Leaderboard)} & \textbf{95.33} & \textbf{98.84} & \textbf{86.42} & \textbf{91.91} & \textbf{87.63} & \textbf{93.06} \\
    \bottomrule
    \multicolumn{7}{l}{\footnotesize $^\dagger$ Novel superclasses were not simulated in internal validation due to limited superclass count.}
  \end{tabular}
  }
\end{table}
