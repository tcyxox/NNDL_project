...
For a model with logits $\mathbf{z} = [z_1, z_2, \ldots, z_C]$ for $C$ classes, the energy score is defined as:
\begin{equation}
    E(\mathbf{x}) = -\log \sum_{i=1}^{C} e^{z_i}
\end{equation}
where lower energy values indicate higher confidence that the sample belongs to a known class.
